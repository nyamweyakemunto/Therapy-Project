...
// import { FiSearch, FiFilter, FiX, FiStar, FiMapPin, FiClock, FiDollarSign } from 'react-icons/fi';
...
const TherapistSearch = () => {
    ...
    const fetchTherapists = async () => {
      try {
        ...
        const transformedTherapists = response.data.map(therapist => ({
          ...
          photo: therapist.profile_picture || `https://randomuser.me/api/portraits/${therapist.gender === 'male' ? 'men' : 'women'}/${Math.floor(Math.random() * 100)}.jpg`,
          bio: therapist.bio || 'Specializing in maternal mental health with a compassionate approach.'
        }));
  ...
  useEffect(() => {
    ...
    if (filters.specialties.length > 0) {
      results = results.filter(therapist =>
        filters.specialties.some(spec => therapist.specialty.includes(spec))
      );
    }
  ...
  // Group therapists into pairs for 2-column layout
  const groupedTherapists = filteredTherapists.reduce((result, therapist, index) => {
    if (index % 2 === 0) {
      result.push([therapist]);
    } else {
      result[result.length - 1].push(therapist);
    }
    return result;
  }, []);
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          {!loading && filteredTherapists.length === 0 ? (
            ...
            <div className="space-y-6">
              {groupedTherapists.map((therapistPair, index) => (
                <div key={index} className="grid md:grid-cols-2 gap-6">
                  {therapistPair.map(therapist => (
                    <div key={therapist.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                      <div className="md:flex">
                        <div className="md:w-1/4 p-6 bg-indigo-50 flex flex-col items-center justify-center">
                          <img 
